...
The export uses the Responses API with `gpt-6-astra`, medium reasoning, structured output, background processing, and `store: false`. The page polls the background response until it is ready, so a long review is not tied to one server request. OpenAI temporarily retains a background response while it is available for polling. Set `OPENAI_API_KEY` on the server and, optionally, `OPENAI_TRADE_REVIEW_MODEL` to override this export's model with a compatible reasoning/vision/structured-output model. The older `OPENAI_REVIEW_MODEL` setting still belongs to the separate watchlist review; it does not override this export. Model access must be enabled for the API project. This quality-oriented export costs more and can take longer than the previous mini-model version.
...
Evidence is not silently truncated. Oversized requests fail with instructions to narrow the filters or reduce active example charts/sources. Repeated comparison charts are loaded once and sent to the model once with every associated trade ID, oversized images are normalized to a readable review resolution, and image/excursion preparation runs with bounded concurrency. Current safeguards are 1,000,000 characters of text context, 450 unique image parts, and a request below 45 MiB; image decoding is limited to 20 MiB/40 megapixels per input. Each submit, status, and finalization request has its own bounded deadline, while Astra continues independently between polls. A refusal, incomplete response, invalid trade references, or missing work priorities fails the export instead of producing a partial report.
