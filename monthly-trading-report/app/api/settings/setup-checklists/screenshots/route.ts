...
export async function POST(request: Request) {
  ...
  if (!exampleId) return NextResponse.json({ error: "A strategy example is required." }, { status: 400 });
  ...
  try {
    return NextResponse.json(
      await saveCamJournalScreenshot("setup-strategy-example", exampleId, file.name, file.type, Buffer.from(await file.arrayBuffer()))
    );
  } catch (error) {
    return NextResponse.json(
