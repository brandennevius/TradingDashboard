...
import { getBrandenPortfolioSettings, listBrandenCalendarTrades } from "@/lib/store";

export async function GET() {
  const logTiming = createApiTimer("/api/journal/branden/calendar");
  ...
  try {
    const [trades, portfolioSettings] = await Promise.all([listBrandenCalendarTrades(), getBrandenPortfolioSettings()]);
