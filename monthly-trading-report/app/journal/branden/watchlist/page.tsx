            ...
            {selectedItem ? (
              <section className="watchlist-detail">
                ...
                <div className="watchlist-fields watchlist-primary-fields">
                  ...
                  <label>Setup<select value={selectedItem.setupTag} disabled={!canEdit} onChange={(event) => chooseSetup(selectedItem, event.target.value)}><option value="">Select setup</option>{templates.filter((template) => !template.archived || template.setupName === selectedItem.setupTag).map((template) => <option key={template.id} value={template.setupName}>{template.setupName}</option>)}</select></label>
