...
export default function BrandenTimeStopPage() {
  ...
  useEffect(() => {
    ...
    loadPageData().catch((loadError) => {
      if (!cancelled) {
        setError(loadError instanceof Error ? loadError.message : "Could not load time stop.");
        setIsLoading(false);
      }
    });
    ...
    <div className="branden-journal-content">
        ...
        <header className="branden-route-header">
          ...
          <nav className="branden-route-nav" aria-label="Branden journal pages">
            ...
            <Link href="/journal/branden/open-positions">Open Positions</Link>
            <Link className="active" href="/journal/branden/time-stop">Time Stop</Link>
          </nav>
        </header>

        {status ? <p className="status trade-log-status">{status}</p> : null}
        {error ? <p className="status error">{error}</p> : null}
        {isLoading ? <p className="status">Loading time stop...</p> : null}

        {!isLoading && !error ? <TimeStopMonitor trades={brandenTrades} activePortfolio={activePortfolio} /> : null}
      </div>
  );
}
