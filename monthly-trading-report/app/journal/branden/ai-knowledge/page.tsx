...
export default function BrandenSetupBuilderPage() {
  ...
  return (
    <section className="branden-journal-main">
        <section className="trade-entry-form setup-settings-page" aria-label="AI knowledge">
          <div className="trade-panel-heading">
            <div>
              <h3>AI Knowledge</h3>
              <span>Manage setup-specific strategy documents and visual examples used by AI reviews.</span>
            </div>
            <Link className="trade-muted-button" href="/journal/branden/setup-builder">Setup Builder</Link>
          </div>
          {isLoading ? <p className="status">Loading AI knowledge...</p> : null}
          {error ? <p className="status error">{error}</p> : null}
          {!canEdit && user ? <p className="muted">Read-only access. AI knowledge sources and examples cannot be changed.</p> : null}
            ...
            <fieldset className="setup-builder-fieldset" disabled={!canEdit}>
              ...
              {canEdit ? (
                <div className="grade-band-actions">
                  ...
                  <button type="button" onClick={saveSetupTemplates}>Save AI Knowledge</button>
